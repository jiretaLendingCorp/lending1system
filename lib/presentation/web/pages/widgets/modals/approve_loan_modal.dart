...
class ApproveLoanModal extends StatefulWidget {
  ...
  const ApproveLoanModal(
      {super.key, required this.loan, required this.onSuccess});
...
class _ApproveLoanModalState extends State<ApproveLoanModal> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final actorRes =
          await sb.from('users').select('id').eq('auth_id', userId!).single();
      ...
      await sb.functions.invoke('send-notification', body: {
        ...
        'notification_type':
            _action == 'approve' ? 'loan_approved' : 'loan_rejected',
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                'Loan ${_action == 'approve' ? 'approved' : 'rejected'} successfully'),
            backgroundColor:
                _action == 'approve' ? AppColors.success : AppColors.error,
          ),
        );
      }
    } catch (e) {
              ...
              children: [
                ...
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: _action == 'approve'
                        ? AppColors.success
                        : AppColors.error,
                  ),
                  child: _loading
                      ...
                      : Text(
                          _action == 'approve' ? 'Approve Loan' : 'Reject Loan',
                          style: const TextStyle(
                              color: Colors.white,
                              fontFamily: 'Poppins',
                              fontWeight: FontWeight.w600)),
...
class _AssignCIModalState extends State<AssignCIModal> {
  ...
  Future<void> _loadRiders() async {
    ...
    final data = await sb
        .from('riders')
        .select(
            'id, rider_code, is_available, users:user_id(first_name, last_name)')
  ...
  Future<void> _assign() async {
    ...
    try {
      ...
      await sb.from('ci_assignments').insert({
        ...
        'instructions': _instructCtrl.text.trim().isEmpty
            ? null
            : _instructCtrl.text.trim(),
      });

      // Update loan status
      await sb
          .from('loans')
          .update({'loan_status': 'under_ci'}).eq('id', widget.loanId);

      // Notify rider
      final rider = _riders.firstWhere((r) => r['id'] == _selectedRiderId,
          orElse: () => {});
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('CI assigned successfully'),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Assign CI Rider',
                          style: TextStyle(
                              fontFamily: 'Poppins',
                              fontSize: 18,
                              fontWeight: FontWeight.w700)),
                      Text(widget.loanCode,
                          style: const TextStyle(
                              fontFamily: 'Poppins',
                              fontSize: 12,
                              color: AppColors.primary500,
            ...
            // Rider dropdown
            const Text('Select Available Rider *',
                style: TextStyle(
                    fontFamily: 'Poppins',
                    fontSize: 13,
                    fontWeight: FontWeight.w600)),
            ...
            _loadingRiders
                ...
                : DropdownButtonFormField<String>(
                    ...
                    validator: (v) =>
                        v == null ? 'Please select a rider' : null,
                  ),
            ...
            TextFormField(
              ...
              maxLines: 3,
              decoration: const InputDecoration(
                  hintText: 'Special instructions for the rider…'),
            ),
